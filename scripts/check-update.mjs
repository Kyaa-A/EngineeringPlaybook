...
const CONFIG = {
  claude: {
    ...
    remote: 'https://raw.githubusercontent.com/Kyaa-A/EngineeringPlaybook/main/.claude-plugin/plugin.json',
    message(installed, latest) {
      return `engineer-project ${installed} -> ${latest} update available\nRun:\n  claude plugin marketplace update engineering-playbook\n  claude plugin update engineer-project@engineering-playbook\nThen restart Claude Code or run /reload-plugins.`;
    },
  },
  codex: {
    manifest: '.codex-plugin/plugin.json',
    remote: 'https://raw.githubusercontent.com/Kyaa-A/EngineeringPlaybook/main/.codex-plugin/plugin.json',
    message(installed, latest) {
      return `engineer-project ${installed} -> ${latest} update available\nRun:\n  codex plugin marketplace upgrade engineering-playbook\n  codex plugin remove engineer-project@engineering-playbook\n  codex plugin add engineer-project@engineering-playbook\nThen start a new Codex session.`;
    },
  },
};
...
function isNewer(current, latest) {
  for (let index = 0; index < 3; index += 1) {
    if (latest[index].length !== current[index].length) return latest[index].length > current[index].length;
...
function remoteUrl(config) {
  if (process.env.NODE_ENV === 'test' && process.env.ENGINEER_PROJECT_UPDATE_TEST_URL) {
    const testUrl = new URL(process.env.ENGINEER_PROJECT_UPDATE_TEST_URL);
...
async function freshCache(data, platform, installed, ttlMs) {
  ...
  return cache.platform === platform && cache.version === installed && Number.isFinite(cache.checkedAt)
    && Date.now() - cache.checkedAt >= 0 && Date.now() - cache.checkedAt < ttlMs;
}
...
async function writeCache(data, platform, installed) {
  ...
  try {
    ...
    await handle.close();
    handle = undefined;
    const windowsReplacement = process.platform === 'win32' || (process.env.NODE_ENV === 'test' && process.env.ENGINEER_PROJECT_UPDATE_TEST_WINDOWS_REPLACE === '1');
...
async function main() {
  if (process.env.ENGINEER_PROJECT_UPDATE_CHECK === '0') return;
  ...
  if (manifest.name !== 'engineer-project') return;
  const current = parseVersion(manifest.version);
  if (!current) return;
  const ttlMs = testNumber('ENGINEER_PROJECT_UPDATE_TTL_MS', DEFAULT_TTL_MS);
  try { if (await freshCache(args.data, args.platform, manifest.version, ttlMs)) return; } catch {}
  const timeoutMs = testNumber('ENGINEER_PROJECT_UPDATE_TIMEOUT_MS', DEFAULT_TIMEOUT_MS, DEFAULT_TIMEOUT_MS);
  const response = await fetch(remoteUrl(config), { signal: AbortSignal.timeout(timeoutMs) });
  if (!response.ok) return;
  const remote = await response.json();
  if (remote?.name !== 'engineer-project') return;
  const latest = parseVersion(remote.version);
  if (!latest) return;
  try { await writeCache(args.data, args.platform, manifest.version); } catch {}
